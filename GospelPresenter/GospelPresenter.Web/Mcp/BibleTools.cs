...
[McpServerToolType]
public sealed class BibleTools(IBibleService bibleService)
{
    [McpServerTool(Name = "list_bibles"), Description("List all available Bible translations")]
    public string ListBibles()
    {
        return JsonSerializer.Serialize(bibleService.Bibles.Select(b => new
        {
            b.Id,
            b.Name
        }));
    ...
    {
        var chapters = bibleService.GetChapters(bibleId, bookId);
    ...
    {
        var verses = bibleService.GetVerses(bibleId, bookId, chapter);
    ...
    {
        var results = bibleService.Search(bibleId, query).Take(50);
