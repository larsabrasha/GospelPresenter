...
public static class BibleBookNames
{
    ...
    private static readonly FrozenDictionary<string, string> EnglishNames = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
    {
    ...
    public static string GetEnglishName(string bookCode) =>
        EnglishNames.GetValueOrDefault(bookCode.ToUpperInvariant()) ?? bookCode;

    public static string GetLocalizedName(string bookCode, string culture) =>
        culture.StartsWith("sv", StringComparison.OrdinalIgnoreCase)
            ? GetSwedishName(bookCode)
            : GetEnglishName(bookCode);
